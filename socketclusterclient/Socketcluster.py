...
class socket(Emitter.emitter):
    async def emitack(self, event, object, ack):
    ...
    async def unsubscribeack(self, channel, ack):
        subscribeobject = json.loads('{}')
        subscribeobject["event"] = "#unsubscribe"
        subscribeobject["data"] = channel
        subscribeobject["cid"] = self.getandincrement()
        ...
        if message == "#1":
            ...
            if result == 1:
                ...
                if self.OnAuthentication is not None:
                    ...
                    await self.OnAuthentication(self, dataobject["isAuthenticated"])
                self.subscribechannels()
            elif result == 2:
                await self.execute(dataobject["channel"], dataobject["data"])
                ...
                if rid in self.acks:
                    ...
                    if tuple is not None:
                        ack = tuple[1]
                        await ack(tuple[0], mainobject["error"], mainobject["data"])
                    else:
    ...
    async def connect(self, sslopt=None, http_proxy_host=None, http_proxy_port=None):
